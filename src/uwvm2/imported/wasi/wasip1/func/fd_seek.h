...
UWVM_MODULE_EXPORT namespace uwvm2::imported::wasi::wasip1::func
{
        ...
        ::uwvm2::imported::wasi::wasip1::abi::wasi_void_ptr_t new_offset_ptrsz) noexcept
    {
        ...
        // Rights check: seek uses seek permission
        if((curr_fd.rights_base & ::uwvm2::imported::wasi::wasip1::abi::rights_t::right_fd_seek) !=
...
// This will be checked at runtime.
        ...
        ::fast_io::seekdir seek_whence;  // no initialize

        switch(whence)
        {
            case ::uwvm2::imported::wasi::wasip1::abi::whence_t::whence_set:
            {
                seek_whence = ::fast_io::seekdir::beg;
                break;
            }
            case ::uwvm2::imported::wasi::wasip1::abi::whence_t::whence_cur:
            {
                seek_whence = ::fast_io::seekdir::cur;
                break;
            }
            case ::uwvm2::imported::wasi::wasip1::abi::whence_t::whence_end:
            {
                seek_whence = ::fast_io::seekdir::end;
                break;
            }
            [[unlikely]] default:
            {
        ...
        using underlying_offset_type_t = ::std::underlying_type_t<::std::remove_cvref_t<decltype(offset)>>;

        auto const underlying_offset{static_cast<underlying_offset_type_t>(offset)};

        if constexpr(::std::numeric_limits<underlying_offset_type_t>::max() > ::std::numeric_limits<::fast_io::intfpos_t>::max())
        {
            if(underlying_offset > ::std::numeric_limits<::fast_io::intfpos_t>::max()) { return ::uwvm2::imported::wasi::wasip1::abi::errno_t::eoverflow; }
        }

        if constexpr(::std::numeric_limits<underlying_offset_type_t>::lowest() < ::std::numeric_limits<::fast_io::intfpos_t>::lowest())
        {
            if(underlying_offset < ::std::numeric_limits<::fast_io::intfpos_t>::lowest()) { return ::uwvm2::imported::wasi::wasip1::abi::errno_t::eoverflow; }
        }
...
# endif
        {
            new_offset_fpos = ::fast_io::operations::io_stream_seek_bytes(file_fd, int_fpos, seek_whence);
        }
# ifdef UWVM_CPP_EXCEPTIONS
            ...
            switch(e.domain)
            {
                case ::fast_io::win32_domain_value:
                {
                    switch(e.code)
                    {
                        ...
                        case 267uz /*ERROR_DIRECTORY*/: return ::uwvm2::imported::wasi::wasip1::abi::errno_t::espipe;
...
# endif
        ...
        {
            if(new_offset_fpos > ::std::numeric_limits<underlying_new_offset_type_t>::max())
            {
        ...
        ::uwvm2::imported::wasi::wasip1::memory::store_basic_wasm_type_to_memory_wasm32(memory,
                                                                                        new_offset_ptrsz,
                                                                                        static_cast<underlying_new_offset_type_t>(new_offset_fpos));
