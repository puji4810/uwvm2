...
int main()
{
    ...
    // Case 18: symlink to absolute path (e.g., /etc) -> eperm
    {
        ...
        auto const ret = ::uwvm2::imported::wasi::wasip1::func::path_create_directory_wasm64(
            env, static_cast<wasi_posix_fd_wasm64_t>(3), p, static_cast<wasi_size_wasm64_t>(sizeof(u8"pcd64_abs/hack") - 1u));
        if(ret != errno_wasm64_t::eperm) { ::fast_io::fast_terminate(); }
    ...
    // Case 19: two-node symlink loop A<->B -> eloop
    {
        ...
        auto const ret = ::uwvm2::imported::wasi::wasip1::func::path_create_directory_wasm64(
            env, static_cast<wasi_posix_fd_wasm64_t>(3), p, static_cast<wasi_size_wasm64_t>(sizeof(u8"pcd64_loop2_a/x") - 1u));
        if(ret != errno_wasm64_t::eloop) { ::fast_io::fast_terminate(); }
    ...
    // Case 20: symlink to ".." inside subdir -> esuccess (pcd64_escape_a/up/x -> x)
    {
        ...
        auto const ret = ::uwvm2::imported::wasi::wasip1::func::path_create_directory_wasm64(
            env, static_cast<wasi_posix_fd_wasm64_t>(3), p, static_cast<wasi_size_wasm64_t>(sizeof(u8"pcd64_escape_a/up/x") - 1u));
        if(ret != errno_wasm64_t::esuccess) { ::fast_io::fast_terminate(); }
    ...
    // Case 21: permission boundary on parent dir -> eacces/eperm
    {
        ...
        auto const ret = ::uwvm2::imported::wasi::wasip1::func::path_create_directory_wasm64(
            env, static_cast<wasi_posix_fd_wasm64_t>(3), p, static_cast<wasi_size_wasm64_t>(sizeof(u8"pcd64_ro_a/no") - 1u));
    ...
    // Case 22: symlink to a file as intermediate -> enotdir
    {
        ...
        auto const ret = ::uwvm2::imported::wasi::wasip1::func::path_create_directory_wasm64(
            env, static_cast<wasi_posix_fd_wasm64_t>(3), p, static_cast<wasi_size_wasm64_t>(sizeof(u8"pcd64_link_to_file/x") - 1u));
        if(ret != errno_wasm64_t::enotdir) { ::fast_io::fast_terminate(); }
