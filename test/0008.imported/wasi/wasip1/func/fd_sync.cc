...
#include <uwvm2/imported/wasi/wasip1/func/fd_sync.h>
...
int main()
{
    ...
    // Case 1: enotcapable when rights do not include right_fd_sync
    {
        ...
        fd1.wasi_fd.ptr->wasi_fd_storage.storage.file_fd
            ...
            .file
#endif
            = ::fast_io::native_file{u8"test_fd_sync.tmp", ::fast_io::open_mode::out};

        auto const ret = ::uwvm2::imported::wasi::wasip1::func::fd_sync(env, static_cast<wasi_posix_fd_t>(4));
        if(ret != errno_t::enotcapable)
        {
            ::fast_io::io::perrln(::fast_io::u8err(), u8"fd_sync: expected enotcapable when rights missing");
    ...
    // Case 2: esuccess when rights ok and fd valid
    {
        ...
        fd3.wasi_fd.ptr->wasi_fd_storage.storage.file_fd
            ...
            .file
#endif
            = ::fast_io::native_file{u8"test_fd_sync_ok.tmp", ::fast_io::open_mode::out};

        auto const ret = ::uwvm2::imported::wasi::wasip1::func::fd_sync(env, static_cast<wasi_posix_fd_t>(3));
        if(ret != errno_t::esuccess)
        {
            ::fast_io::io::perrln(::fast_io::u8err(), u8"fd_sync: expected esuccess with valid rights and handle");
            ::fast_io::fast_terminate();
        }
    }
    ...
    {
        auto const ret = ::uwvm2::imported::wasi::wasip1::func::fd_sync(env, static_cast<wasi_posix_fd_t>(-1));
        if(ret != errno_t::ebadf)
        {
            ::fast_io::io::perrln(::fast_io::u8err(), u8"fd_sync: expected ebadf for negative fd");
            ::fast_io::fast_terminate();
        }
    }
    ...
    // Case 4: ebadf after fd has been closed
    {
        ...
        fd2.wasi_fd.ptr->wasi_fd_storage.storage.file_fd
            ...
            .file
#endif
            = ::fast_io::native_file{u8"test_fd_close_then_sync.tmp", ::fast_io::open_mode::out};
        ...
        {
            ::fast_io::io::perrln(::fast_io::u8err(), u8"fd_sync: close before sync expected esuccess");
            ::fast_io::fast_terminate();
        }

        auto const ret = ::uwvm2::imported::wasi::wasip1::func::fd_sync(env, static_cast<wasi_posix_fd_t>(2));
        if(ret != errno_t::ebadf)
        {
            ::fast_io::io::perrln(::fast_io::u8err(), u8"fd_sync: expected ebadf after fd_close");
            ::fast_io::fast_terminate();
        }
    }
}
