...
int main()
{
    ...
    // Case 1: ebadf when fd is negative
    {
        ...
        {
            ::fast_io::io::perrln(::fast_io::u8err(), u8"fd_close: expected ebadf for negative fd");
    ...
    // Case 2: esuccess when closing a valid opened fd
    {
        ...
        fd1.wasi_fd.ptr->wasi_fd_storage.storage.file_fd = ::fast_io::native_file{u8"test_fd_close.tmp", ::fast_io::open_mode::out};
        fd1.rights_base = static_cast<rights_t>(-1);

        auto const ret = ::uwvm2::imported::wasi::wasip1::func::fd_close(env, static_cast<wasi_posix_fd_t>(1));
        if(ret != errno_t::esuccess)
        {
            ::fast_io::io::perrln(::fast_io::u8err(), u8"fd_close: expected esuccess for valid close");
            ::fast_io::fast_terminate();
        }
    }
    ...
    // Case 3: double close should return ebadf
    {
        ...
        fd2.wasi_fd.ptr->wasi_fd_storage.storage.file_fd = ::fast_io::native_file{u8"test_fd_close.tmp", ::fast_io::open_mode::out};
        ...
        if(first != errno_t::esuccess)
        {
            ::fast_io::io::perrln(::fast_io::u8err(), u8"fd_close: first close expected esuccess");
            ::fast_io::fast_terminate();
        }

        auto const second = ::uwvm2::imported::wasi::wasip1::func::fd_close(env, static_cast<wasi_posix_fd_t>(2));
        if(second != errno_t::ebadf)
        {
            ::fast_io::io::perrln(::fast_io::u8err(), u8"fd_close: second close expected ebadf");
            ::fast_io::fast_terminate();
        }
    }
    ...
    {
        // choose a large fd that is not in opens
        constexpr wasi_posix_fd_t kOutOfRangeFd{12345};
        // populate renumber_map with a valid entry
        env.fd_storage.renumber_map.emplace(kOutOfRangeFd, ::uwvm2::imported::wasi::wasip1::fd_manager::wasi_fd_unique_ptr_t{});
        auto const ret = ::uwvm2::imported::wasi::wasip1::func::fd_close(env, kOutOfRangeFd);
        if(ret != errno_t::esuccess)
        {
            ::fast_io::io::perrln(::fast_io::u8err(), u8"fd_close: renumber_map path expected esuccess");
            ::fast_io::fast_terminate();
        }
    }
}
